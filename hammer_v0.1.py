...
def update_goal():
    ...
    # STEP 1: Pull everything out of the filing cabinet
    all_goals = []
    with open("hammer_db.csv", "r") as file:
        reader = csv.DictReader(file)
        for row in reader:
    ...
    # STEP 2: Find the specific piece of paper and change it
    ...
    for goal in all_goals:
        if goal["Task Name"] == target_name:
            ...
            goal["Status"] = new_status  # This updates the dictionary in our active memory!
            found = True
            break
            
    if not found:
        print("Error: Task not found in database.")
        return # Eject button if they typo the name
        
    # STEP 3 & 4: Burn the cabinet, build a new one, and dump the files back in
    with open("hammer_db.csv", "w") as file:
        writer = csv.DictWriter(file, fieldnames=headers)
        writer.writeheader()
        
        # NOTE the 's' at the end! writerows() takes an entire list of dictionaries and writes them all at once.
...
def main_menu():
    while True:
        ...
        if choice == "1":
            view_all_goals()  # This triggers your function!
        elif choice == "2":
            add_new_goal()
        elif choice == "3":
            update_goal()
